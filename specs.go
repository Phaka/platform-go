...
const (
    HypervisorKindVMware HypervisorKind = "vSphere"
)

type Hypervisors map[HypervisorKind]*Hypervisor
...
const (
    BootMethodKindHttp   BootMethodKind = "http"
    BootMethodKindCdrom  BootMethodKind = "cdrom"
    BootMethodKindFloppy BootMethodKind = "floppy"
)
...
type BootMethods map[BootMethodKind]*BootMethod
...
func (o *OperatingSystem) UnmarshalYAML(value *yaml.Node) error {
    ...
    if o.Hypervisors == nil {
        ...
        hypervisorKinds := AllHypervisorKinds()
        for _, kind := range hypervisorKinds {
    ...
    if o.BootMethods == nil {
        ...
        bootMethods := BootMethodKinds()

        for _, method := range bootMethods {
            if _, ok := o.BootMethods[method]; !ok {
                o.BootMethods[method] = defaultBootMethods[method]
            }
        }
    }
...
func AllHypervisorKinds() []HypervisorKind {
    return []HypervisorKind{
        HypervisorKindVMware,
    }
}

func BootMethodKinds() []BootMethodKind {
    return []BootMethodKind{
        BootMethodKindHttp,
        BootMethodKindCdrom,
